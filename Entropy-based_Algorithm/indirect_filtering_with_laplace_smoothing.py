...
def dfr(a, log):
    ...
    for event in acts:
        trace1 = [a, event]
        trace2 = [a]
        result.append(((1 / num) + occurrences_time(trace1, l_temp)) / (((1 / num) * (num + 1)) + occurrences_time(trace2, l_temp)))
...
def h3(log):
    ...
    acts = activities(log)
    for a in acts:
        result = result + h2(a, log)
    return result


# l是dict{}
def arg_min(acts, log):
    max_h = float("inf")
    result = ''
    for a in acts:
        if h3(l_temp_update(acts, a, log)) < max_h:
            result = a
    return result
...
def indirect_filtering_with_laplace_smoothing(log):
    l_temp = {}
